...
1.요구사항
-꽃 테이블과 화분테이블 2개가 필요하고, 꽃을 구매할 떄 화분 테이블도 같이 구매한다.
꽃은 꽃이름과 색깔, 가격이 존재한다.
-화분은 제품번호, 화문색깔, 화분모양, 꽃이름이 존재한다.
  */
...
CREATE TABLE "ORDER"(
	...
	ORDER_DATE DATE,
	USER_ID VARCHAR2(100), --참조하려고 하는 컬럼의 타입과 크기가 일치해야한다.
	PRODUCT_NUM NUMBER,
...
--10번 및 30번 부서에 속하는 모든 사원 중 급여가 1500원을 넘는 사원의 사원번호,이름 및 급여를 조회하세요
